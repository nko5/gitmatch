var config = require('./config');
var request = require('request');
var fs = require('fs');
var path = require('path');
var redis = require('redis');
var GitHubApi = require('github');
var repository = {};

var github = new GitHubApi({
  // required
  version: '3.0.0',
  // optional
  protocol: 'https',
  host: 'api.github.com',
  timeout: 5000,
  headers: {
    'user-agent': 'BarcelonaJS' // GitHub is happy with a unique user agent
  }
});

if (process.env.NODE_ENV === 'development') {
  github.debug = true;
}

repository.getRepo = function(userName, repoName) {
  return new Promise(function(resolve, reject) {
    var client = redis.createClient(
      config.redis.url, {}
    );
    client.get('repo:' + userName + ':' + repoName, function(error, cachedRepo) {
      if (!error && cachedRepo) {
        var repoJson = JSON.parse(cachedRepo);
        client.quit();
        resolve(repoJson);
      } else {
        github.repos.get({
          user: userName,
          repo: repoName
        }, function(error, liveRepo) {
          if (!liveRepo) {
            client.quit();
            reject('json error');
          } else {
            delete liveRepo.meta;
            client.set('repo:' + userName + ':' + repoName, JSON.stringify(liveRepo), function() {
              client.quit();
              resolve(liveRepo);
            });
          }
        });
      }
    });
  });
};

repository.hasIssues = function(repo) {
  return new Promise(function(resolve, reject) {
    if (repo.open_issues_count <= 5) {
      reject('not enough issues');
    } else {
      resolve('issues ok');
    }
  });
};

repository.getPackageJson = function(repo) {
  return new Promise(function(resolve, reject) {
    var client = redis.createClient(
      config.redis.url, {}
    );
    var url = repo.contents_url.replace('{+path}', 'package.json?ref=master');
    client.get('package:' + url, function(error, cachedRepo) {
      if (!error && cachedRepo) {
        var repoJson = JSON.parse(cachedRepo);
        client.quit();
        resolve(repoJson);
      } else {
        var content = '';
        var headers = {
          'User-Agent': 'BarcelonaJS'
        };

        var options = {
          uri: url,
          method: 'GET',
          headers: headers
        };

        if (!repo) {
          client.quit();
          reject('empty');
        }

        request(options)
          .on('response', function(response) {
            if (response.statusCode === 404) {
              client.quit();
              reject('package.json not found');
            }
          })
          .on('data', function(chunk) {
            content += chunk;
          })
          .on('end', function() {
            var contentJson;
            content = content.toString('utf8');

            try {
              contentJson = JSON.parse(content);
            } catch (e) {
              client.quit();
              reject(e);
            }

            if (!contentJson) {
              client.quit();
              reject('json error');
            } else {
              var body = new Buffer(contentJson.content, contentJson.encoding);
              client.set('package:' + contentJson.url, body.toString('utf8'), function() {
                client.quit();
                resolve(JSON.parse(body.toString('utf8')));
              });
            }
          });
      }
    });
  });
};

repository.checkContributingmd = function(repo) {
  return new Promise(function(resolve, reject) {
    if (!repo) {
      reject('empty');
    }

    var url = repo.contents_url.replace('{+path}', 'CONTRIBUTING.md?ref=master');
    var headers = {
      'User-Agent': 'BarcelonaJS'
    };

    var options = {
      uri: url,
      method: 'GET',
      headers: headers
    };

    request(options)
      .on('response', function(response) {
        if (response.statusCode === 404) {
          reject('contributing.md not found');
        } else {
          resolve('contributing ok');
        }
      });
  });
};

repository.createContributingmd = function(req, repo) {
  return new Promise(function(resolve, reject) {
    github.authenticate({
      type: 'oauth',
      token: req.user.accessToken
    });

    var file = fs.readFileSync(path.join(__dirname, 'data', 'CONTRIBUTING.md'));

    github.repos.createContent({
      user: req.user.profile.username,
      repo: repo,
      path: 'CONTRIBUTING.md',
      content: file,
      message: 'Created CONTRIBUTING.md'
    }, function(error) {
      if (error) {
        reject(error);
      }
      resolve('contributing ok');
    });
  });
};

repository.checkRepo = function(repo) {
  var hasIssues = repository.hasIssues(repo);
  var hasContribuingMd = repository.checkContributingmd(repo);
  var hasPackageJson = repository.getPackageJson(repo);

  // TODO: have more checks here
  return Promise.all([hasContribuingMd, hasPackageJson, hasIssues]);
};

module.exports = exports = repository;
